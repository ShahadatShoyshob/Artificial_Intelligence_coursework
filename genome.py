...
class Genome():
    ...
    def genome_to_links(gdicts):
        ...
        for gdict in gdicts:
            ...
            # Default to box shape if no shape specified in genome
            link_shape = gdict.get("link-shape", "Box")

            # Handle evolvability for specific parameters
            evolvable_length = gdict.get("evolvable-length", True)
            evolvable_radius = gdict.get("evolvable-radius", True)
            evolvable_motor = gdict.get("evolvable-motor", True)

            # Use default or fixed values if not evolvable
            link_length = gdict["link-length"] if evolvable_length else 1.0
            link_radius = gdict["link-radius"] if evolvable_radius else 0.1

            # Handle motor evolvability
            control_waveform = gdict["control-waveform"] if evolvable_motor else 0.5
            control_amp = gdict["control-amp"] if evolvable_motor else 1.0
            control_freq = gdict["control-freq"] if evolvable_motor else 1.0
            ...
            link = URDFLink(
                ...
                link_shape=link_shape,  # New shape attribute
            )
            links.append(link)

            # Add current link to parent list unless it's the first link
            if link_ind != 0:
                parent_names.append(link_name)
            link_ind += 1

        # Fix the first link to link to "None"
